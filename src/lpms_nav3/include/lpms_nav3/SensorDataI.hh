...
#define PRINTF_BINARY_PATTERN_INT16_PP PRINTF_BINARY_PATTERN_INT8 " " PRINTF_BINARY_PATTERN_INT8
...
struct IG1InfoI {
    ...
    std::string toString() {
        ...
        ss << std::setw(w) << std::left
           << "IAP check status: " << (iapCheckStatus ? "Ready" : "Not Ready") << "\n";
        return ss.str();
    }
};
...
struct IG1SettingsI {
    ...
    std::string toString() {
        ...
        // general
        ss << "=== General === \n";
        ss << "Transmit Data: " << transmitDataConfig << " ("
           << uint32ToBinaryPP(transmitDataConfig) << ")\n";
        ss << std::setw(w) << std::left << "Sensor ID: " << sensorId << "\n";
        ss << std::setw(w) << std::left << "Data stream freq: " << dataStreamFrequency << "Hz\n";
        ss << std::setw(w) << std::left
           << "Gyro output unit: " << (useRadianOutput ? "radian" : "deg") << "\n";
        ss << std::setw(w) << std::left
           << "Gyro autocalibration: " << (enableGyroAutocalibration ? "Enable" : "Disable")
           << "\n";
        ...
        ss << std::setw(w) << std::left << "Mag CalibrationTimeout: " << magCalibrationTimeout
           << " s\n";
        ...
        ss << std::setw(w) << std::left
           << "Uart data format: " << (uartDataFormat ? "ASCII" : "LPBus") << "\n";
        ss << std::setw(w) << std::left
           << "Uart data precision: " << (uartDataPrecision ? "Floating point" : "Fixed point")
           << "\n";
        ss << std::setw(w) << std::left << "Uart ascii start: " << std::hex << "0x"
           << (int)uartAsciiStart << "\n";
        ss << std::setw(w) << std::left << "Uart ascii stop: " << std::hex << "0x"
           << (int)uartAsciiStop << "\n";
        ss << std::dec;

        ss << "=== CAN === \n";
        ss << std::setw(w) << std::left << "CAN Start ID: " << canStartId << std::hex << "(0x"
           << canStartId << std::dec << ")" << "\n";
        ss << std::setw(w) << std::left << "CAN baudrate: " << canBaudrate << "\n";
        ss << std::setw(w) << std::left
           << "CAN data precision: " << (canDataPrecision ? "Floating point" : "Fixed point")
           << "\n";
        ...
        ss << "=== GPS === \n";
        ss << "GPS Transmit Data0: " << gpsTransmitDataConfig[0] << " ("
           << uint32ToBinaryPP(gpsTransmitDataConfig[0]) << ")\n";
        ss << "GPS Transmit Data1: " << gpsTransmitDataConfig[1] << " ("
           << uint32ToBinaryPP(gpsTransmitDataConfig[1]) << ")\n";
    ...
    std::string uint32ToBinaryPP(uint32_t data) {
        ...
        snprintf(
            binaryFormat, 36, PRINTF_BINARY_PATTERN_INT32_PP, PRINTF_BYTE_TO_BINARY_INT32(data));
