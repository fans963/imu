...
using namespace std;
#ifdef _WIN32
IG1I* APIENTRY IG1Factory() { return new IG1(); }
#else
IG1I* IG1Factory() { return new IG1(); }
#endif
/////////////////////////////////////////////
...
#endif
{
    if (sensorStatus == STATUS_CONNECTING || sensorStatus == STATUS_CONNECTED
...
int IG1::getReconnectCount() { return reconnectCount; }

void IG1::setConnectionInterface(int interface) { connectionInterface = interface; }
...
void IG1::setControlGPIOToggleWaitMs(unsigned int ms) { ctrlGpioToggleWaitMs = ms; }
...
bool IG1::getAutoReconnectStatus(void) { return autoReconnect; };

int IG1::getStatus() { return sensorStatus; };
...
// info
bool IG1::hasInfo() { return hasNewInfo; }
...
// settings
bool IG1::hasSettings() { return hasNewSettings; }
...
int IG1::hasResponse() { return sensorResponseQueue.size(); }
...
bool IG1::isUseRadianOutput() { return sensorSettings.useRadianOutput; }
...
int IG1::getSensorDataQueueSize() { return sensorDataQueueSize; }

int IG1::getFilePages() { return firmwarePages; };

bool IG1::getIsUpdatingStatus() { return connectionState == CONNECTION_STATE_FIRMWARE_UPDATE; };

// Error
std::string IG1::getLastErrMsg() { return errMsg; };

void IG1::setVerbose(int level) { log.setVerbose(level); }
...
int IG1::getSavedImuDataCount() { return savedImuDataCount; }
...
int IG1::getSavedGpsDataCount() { return savedGpsDataCount; }
...
void IG1::processCommandQueue() {
    ...
    if (!commandQueue.empty() && mmCommandTimer.measure() > TIMEOUT_COMMAND_TIMER) {
        ...
        if (!commandQueue.front().sent) {
            ...
            sendCommand(
                commandQueue.front().command, commandQueue.front().dataLength,
                commandQueue.front().data.c);
        ...
        // Process sent command
            ...
            if (mmCommandTimer.measure() > 200000) // .2sec no response, skip
            {
                if (commandQueue.front().expectedResponse != WAIT_IGNORE) {
                    log.e(
                        TAG, "Command timeout. Resending Command : %d Retry: %d\r\n",
            ...
            while (!isStopThread) {
                ...
                case CONNECTION_STATE_WAITING_SENSOR_INFO: {
                    ...
                    if (TDRRetryCount > 3) {
                        ...
                        stringstream ss;
                        ss << "[" + currentDateTime("%Y/%m/%d %H:%M:%S")
                                  + "] Error getting transmit data register\n";
                ...
                case CONNECTION_STATE_TDR_ERROR: sensorStatus = STATUS_CONNECTION_ERROR; break;

                case CONNECTION_STATE_FIRMWARE_UPDATE: sensorStatus = STATUS_CONNECTED; break;

                case CONNECTION_STATE_CONNECTED: sensorStatus = STATUS_CONNECTED; break;
...
bool IG1::parseModbusByte(int n) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        default: packet.rxState = PACKET_START; break;
        }
    }
    return true;
}
...
bool IG1::parseSensorData(const LPPacket& p) {
    ...
    case GET_IMU_DATA: {
        ...
        if (sensorSettings.uartDataPrecision == LPMS_UART_DATA_PRECISION_FIXED_POINT) {
            latestImuData.setData16bit(
                sensorSettings.useRadianOutput, sensorSettings.transmitDataConfig, packet.data);
        } else {
    ...
    case GET_GPS_DATA: {
        ...
        latestGpsData.setData(
            sensorSettings.gpsTransmitDataConfig[0], sensorSettings.gpsTransmitDataConfig[1],
            packet.data);
    ...
    case GET_SENSOR_MODEL:
        ...
        res = "[" + currentDateTime("%Y/%m/%d %H:%M:%S")
            + "] GET DEVICE NAME: " + sensorInfo.deviceName;
        addSensorResponseToQueue(res);
        hasNewInfo = true;
    ...
    case GET_FIRMWARE_INFO:
        ...
        res = "[" + currentDateTime("%Y/%m/%d %H:%M:%S")
            + "] GET FIRMWARE INFO: " + sensorInfo.firmwareInfo;
        addSensorResponseToQueue(res);
        hasNewInfo = true;
    ...
    case GET_SERIAL_NUMBER:
        ...
        res                     = "[" + currentDateTime("%Y/%m/%d %H:%M:%S")
            + "] GET SERIAL NUMBER: " + sensorInfo.serialNumber;
        addSensorResponseToQueue(res);
        hasNewInfo = true;
    ...
    case GET_FILTER_VERSION:
        ...
        res                      = "[" + currentDateTime("%Y/%m/%d %H:%M:%S")
            + "] GET FILTER VERSION: " + sensorInfo.filterVersion;
        addSensorResponseToQueue(res);
        hasNewInfo = true;
    ...
    case GET_IAP_CHECKSTATUS:
        ...
        log.d(
            TAG, "Received GET_IAP_CHECKSTATUS: %s\n", sensorInfo.iapCheckStatus ? "Ready" : "NA");
        break;
    ...
    case GET_DEGRAD_OUTPUT:
        ...
        log.d(
            TAG, "Received GET_DEGRAD_OUTPUT: %s\n",
    ...
    case GET_ENABLE_GYR_AUTOCALIBRATION:
        ...
        log.d(
            TAG, "Received GET_ENABLE_GYR_AUTOCALIBRATION: %i\n",
    ...
    case GET_MAG_CALIBRATION_TIMEOUT:
        ...
        log.d(
            TAG, "Received GET_MAG_CALIBRATION_TIMEOUT: %f\n",
